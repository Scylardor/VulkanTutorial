...
void HelloTriangleApplication::createImageViews()
{
	...
	{
		swapChainImageViews[imgIndex] = createImageView(swapImage, swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
		imgIndex++;
	}
}
...
void HelloTriangleApplication::createGraphicsPipeline()
{
	...
	// the comparison that is performed to keep or discard fragments.
...
void HelloTriangleApplication::createRenderPass()
{
	...
	depthAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE; // for now, TODO ?
	...
	// The attachment description of our resolved MSAA framebuffer
...
void HelloTriangleApplication::createTextureImage()
{
	...
	// To generate mipmaps, we will use the vkCmdBlitImage command.
	// It is considered a transfer operation, so we must inform Vulkan that we intend to use the texture image as both the source and destination of a transfer.
	createImage(texWidth, texHeight, textureMipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
...
void HelloTriangleApplication::createTextureSampler()
{
	...
	samplerInfo.maxLod = (float)textureMipLevels; // TODO : to refactor
                                           ...
                                           VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
	...
	VkResult ok = vkCreateImage(device, &imageInfo, nullptr, &image);
	assert(ok == VK_SUCCESS);
	...
	ok = vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory);
	assert(ok == VK_SUCCESS);
...
void HelloTriangleApplication::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	...
	else if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
	{
		...
		// In the case of depth attachment,
...
void HelloTriangleApplication::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
	...
	// TODO: refactor (dont check device for each mipmap generation)
	...
	for (uint32_t i = 1; i < mipLevels; i++)
	{
		...
		vkCmdBlitImage(commandBuffer,
			...
			1, &blit,
			VK_FILTER_LINEAR); //  to enable interpolation.
	...
	// This barrier transitions the last mip level because this wasn't handled by the loop, since the last mip level is never blitted from.
...
void HelloTriangleApplication::createColorResources()
{
	...
	// using only one mip level, since this is enforced by the Vulkan specification in case of images with more than one sample per pixel.
...
void HelloTriangleApplication::cleanupSwapChain()
{
	...
	// TODO: this is actually useless to do it here...
...
VkSampleCountFlagBits HelloTriangleApplication::getMaxUsableSampleCount() const
{
